...
const PAID_MEDIA_PAGE_SLUG = "service-paid-media";
...
function mergeSections(existing: PageSection[]): PageSection[] {
  ...
  for (const defaults of paidMediaSections) {
...
async function updatePaidMediaPage() {
  if (!PAGE_DEFAULTS) {
    throw new Error("service-paid-media page defaults missing from defaultPages");
  }
  if (!CONTENT_DEFAULTS) {
    throw new Error("paid-media content defaults missing from services");
  }

  await connectDB();

  const pageResult = await Page.updateOne(
    { slug: PAID_MEDIA_PAGE_SLUG },
    { $set: PAGE_DEFAULTS },
    { upsert: true },
  );

  const existingPage = await Page.findOne({ slug: PAID_MEDIA_PAGE_SLUG }).lean<PageRecord>();
  if (existingPage) {
    const sections = mergeSections(existingPage.sections);
    await Page.updateOne({ slug: PAID_MEDIA_PAGE_SLUG }, { $set: { sections } });
    console.log(`Section order: ${sections.map((section) => section.id).join(" → ")}`);
  }

  const contentResult = await Content.updateOne(
    { slug: PAID_MEDIA_CONTENT_SLUG },
    { $set: CONTENT_DEFAULTS },
    { upsert: true },
  );

  console.log(
    `Performance marketing page — matched: ${pageResult.matchedCount}, modified: ${pageResult.modifiedCount}, upserted: ${pageResult.upsertedCount ? "yes" : "no"}`,
  );
  console.log(
    `Performance marketing content — matched: ${contentResult.matchedCount}, modified: ${contentResult.modifiedCount}, upserted: ${contentResult.upsertedCount ? "yes" : "no"}`,
  );

  await disconnectDB();
}

updatePaidMediaPage().catch((error) => {
  console.error(error);
  process.exit(1);
});
